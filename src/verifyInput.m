...
pParam = pfiGraph.Nodes{pRow,'SectionProperties'}{1}.Variables;
isRequired = pfiGraph.Nodes{pRow,'SectionProperties'}{1}.isRequired;
isUsed = any(contains(inGraph.Nodes{:,'Name'},'STEP'));

if (isRequired == true) && (isUsed == false)
    error('"STEP" block is required');
else
    iRow = find(contains(inGraph.Nodes{:,'Name'},'STEP'));
...
pParam = pfiGraph.Nodes{pRow,'SectionProperties'}{1}.Variables;
isRequired = pfiGraph.Nodes{pRow,'SectionProperties'}{1}.isRequired;
isUsed = any(contains(inGraph.Nodes{:,'Name'},'SOLUTION_CONTROL'));

if (isRequired == true) && (isUsed == false)
    error('"SOLUTION_CONTROL" block is required');
else
    iRow = find(contains(inGraph.Nodes{:,'Name'},'SOLUTION_CONTROL'));
    iParam = inGraph.Nodes{iRow,'Parameters'};
end
...
pParam = pfiGraph.Nodes{pRow,'SectionProperties'}{1}.Variables;
isRequired = pfiGraph.Nodes{pRow,'SectionProperties'}{1}.isRequired;
isUsed = any(contains(inGraph.Nodes{:,'Name'},'LINEAR_SOLVE'));

if (isRequired == true) && (isUsed == false)
    error('"LINEAR_SOLVE" block is required');
else
    iRow = find(contains(inGraph.Nodes{:,'Name'},'LINEAR_SOLVE'));
    iParam = inGraph.Nodes{iRow,'Parameters'};
end
...
for idx = 1:length(iRow)
    for ii = 1:length(iParam{idx}.Names)
        ...
        elseif strcmpi(iParam{idx}.Names(ii),'description')
            assert(isstring(iParam{idx}.Values{ii}))
        elseif strcmpi(iParam{idx}.Names(ii),'relative tolerance')
            inGraph.Nodes{iRow,'Parameters'}{1}.Values{ii} = str2double(iParam{idx}.Values{ii});
        elseif strcmpi(iParam{idx}.Names(ii),'solver')
            assert(any(strcmpi(pParam.solver.validValues,iParam{idx}.Values{ii})))
        elseif strcmpi(iParam{idx}.Names(ii),'preconditioner')
            assert(any(strcmpi(pParam.precond.validValues,iParam{idx}.Values{ii})))
        elseif strcmpi(iParam{idx}.Names(ii),'k-space')
            inGraph.Nodes{iRow,'Parameters'}{1}.Values{ii} = str2double(iParam{idx}.Values{ii});
        elseif strcmpi(iParam{idx}.Names(ii),'maximum iterations')
            inGraph.Nodes{iRow,'Parameters'}{1}.Values{ii} = str2double(iParam{idx}.Values{ii});
        end
    end
end
...
pParam = pfiGraph.Nodes{pRow,'SectionProperties'}{1}.Variables;
isRequired = pfiGraph.Nodes{pRow,'SectionProperties'}{1}.isRequired;
isUsed = any(contains(inGraph.Nodes{:,'Name'},'TIME_CONTROL'));

if (isRequired == true) && (isUsed == false)
    error('"TIME_CONTROL" block is required');
else
    iRow = find(contains(inGraph.Nodes{:,'Name'},'TIME_CONTROL'));
    iParam = inGraph.Nodes{iRow,'Parameters'};
end
...
for idx = 1:length(iRow)
    for ii = 1:length(iParam{idx}.Names)
        ...
        elseif strcmpi(iParam{idx}.Names(ii),'description')
            assert(isstring(iParam{idx}.Values{ii}))
        elseif strcmpi(iParam{idx}.Names(ii),'step time')
            inGraph.Nodes{iRow,'Parameters'}{1}.Values{ii} = str2double(iParam{idx}.Values{ii});
        elseif strcmpi(iParam{idx}.Names(ii),'max time control')
            inGraph.Nodes{iRow,'Parameters'}{1}.Values{ii} = str2double(iParam{idx}.Values{ii});
        elseif strcmpi(iParam{idx}.Names(ii),'max time scale factor')
            inGraph.Nodes{iRow,'Parameters'}{1}.Values{ii} = str2double(iParam{idx}.Values{ii});
        end
    end
end
...
pParam = pfiGraph.Nodes{pRow,'SectionProperties'}{1}.Variables;
isRequired = pfiGraph.Nodes{pRow,'SectionProperties'}{1}.isRequired;
isUsed = any(contains(inGraph.Nodes{:,'Name'},'FLOW_DEFINITION'));

if (isRequired == true) && (isUsed == false)
    error('"FLOW_DEFINITION" block is required');
else
    iRow = find(contains(inGraph.Nodes{:,'Name'},'FLOW_DEFINITION'));
    iParam = inGraph.Nodes{iRow,'Parameters'};
end
...
pParam = pfiGraph.Nodes{pRow,'SectionProperties'}{1}.Variables;
isRequired = pfiGraph.Nodes{pRow,'SectionProperties'}{1}.isRequired;
isUsed = any(contains(inGraph.Nodes{:,'Name'},'REYNOLDS'));

if (isRequired == true) && (isUsed == false)
    error('"REYNOLDS" block is required');
else
    iRow = find(contains(inGraph.Nodes{:,'Name'},'REYNOLDS'));
    iParam = inGraph.Nodes{iRow,'Parameters'};
end
...
for idx = 1:length(iRow)
    for ii = 1:length(iParam{idx}.Names)
        ...
        elseif strcmpi(iParam{idx}.Names(ii),'description')
            assert(isstring(iParam{idx}.Values{ii}))
        elseif strcmpi(iParam{idx}.Names(ii),'function')
            assert(any(strcmpi(pParam.precond.validValues,iParam{idx}.Values{ii})))
        elseif strcmpi(iParam{idx}.Names(ii),'value')
            inGraph.Nodes{iRow,'Parameters'}{1}.Values{ii} = str2double(iParam{idx}.Values{ii});
        elseif strcmpi(iParam{idx}.Names(ii),'scale')
            inGraph.Nodes{iRow,'Parameters'}{1}.Values{ii} = str2double(iParam{idx}.Values{ii});
        end
    end
end
...
pParam = pfiGraph.Nodes{pRow,'SectionProperties'}{1}.Variables;
isRequired = pfiGraph.Nodes{pRow,'SectionProperties'}{1}.isRequired;
isUsed = any(contains(inGraph.Nodes{:,'Name'},'CIRCULATION'));

if (isRequired == true) && (isUsed == false)
    error('"CIRCULATION" block is required');
else
    iRow = find(contains(inGraph.Nodes{:,'Name'},'CIRCULATION'));
    iParam = inGraph.Nodes{iRow,'Parameters'};
end
...
pParam = pfiGraph.Nodes{pRow,'SectionProperties'}{1}.Variables;
isRequired = pfiGraph.Nodes{pRow,'SectionProperties'}{1}.isRequired;
isUsed = any(contains(inGraph.Nodes{:,'Name'},'LOG_INFO'));

if (isRequired == true) && (isUsed == false)
    error('"LOG_INFO" block is required');
else
    iRow = find(contains(inGraph.Nodes{:,'Name'},'LOG_INFO'));
    iParam = inGraph.Nodes{iRow,'Parameters'};
end
...
for idx = 1:length(iRow)
    for ii = 1:length(iParam{idx}.Names)
        ...
        elseif strcmpi(iParam{idx}.Names(ii),'description')
            assert(isstring(iParam{idx}.Values{ii}))
        elseif strcmpi(iParam{idx}.Names(ii),'log filename')
            assert(isstring(iParam{idx}.Values{ii}))
        elseif strcmpi(iParam{idx}.Names(ii),'time increment')
            inGraph.Nodes{iRow,'Parameters'}{1}.Values{ii} = str2double(iParam{idx}.Values{ii});
        elseif strcmpi(iParam{idx}.Names(ii),'initial step')
            assert(any(strcmpi(["true","false"],iParam{idx}.Values{ii})))
            inGraph.Nodes{iRow,'Parameters'}{1}.Values{ii} = str2num(char(iParam{idx}.Values{ii}));
        elseif strcmpi(iParam{idx}.Names(ii),'final step')
            assert(any(strcmpi(["true","false"],iParam{idx}.Values{ii})))
            inGraph.Nodes{iRow,'Parameters'}{1}.Values{ii} = str2num(char(iParam{idx}.Values{ii}));
        elseif strcmpi(iParam{idx}.Names(ii),'log data')
            assert(isstring(iParam{idx}.Values{ii}))
        end
    end
end
...
pParam = pfiGraph.Nodes{pRow,'SectionProperties'}{1}.Variables;
isRequired = pfiGraph.Nodes{pRow,'SectionProperties'}{1}.isRequired;
isUsed = any(contains(inGraph.Nodes{:,'Name'},'IN_SITU'));

if (isRequired == true) && (isUsed == false)
    error('"IN_SITU" block is required');
else
    iRow = find(contains(inGraph.Nodes{:,'Name'},'IN_SITU'));
    iParam = inGraph.Nodes{iRow,'Parameters'};
end
...
for idx = 1:length(iRow)
    for ii = 1:length(iParam{idx}.Names)
        ...
        elseif strcmpi(iParam{idx}.Names(ii),'description')
            assert(isstring(iParam{idx}.Values{ii}))
        elseif strcmpi(iParam{idx}.Names(ii),'plotFlow')
            assert(any(strcmpi(["true","false"],iParam{idx}.Values{ii})))
            inGraph.Nodes{iRow,'Parameters'}{1}.Values{ii} = str2num(char(iParam{idx}.Values{ii}));
        elseif strcmpi(iParam{idx}.Names(ii),'plotState')
            assert(any(strcmpi(["true","false"],iParam{idx}.Values{ii})))
            inGraph.Nodes{iRow,'Parameters'}{1}.Values{ii} = str2num(char(iParam{idx}.Values{ii}));
        elseif strcmpi(iParam{idx}.Names(ii),'saveFlow')
            assert(any(strcmpi(["true","false"],iParam{idx}.Values{ii})))
            inGraph.Nodes{iRow,'Parameters'}{1}.Values{ii} = str2num(char(iParam{idx}.Values{ii}));
        elseif strcmpi(iParam{idx}.Names(ii),'saveState')
            assert(any(strcmpi(["true","false"],iParam{idx}.Values{ii})))
            inGraph.Nodes{iRow,'Parameters'}{1}.Values{ii} = str2num(char(iParam{idx}.Values{ii}));
