...
pU = (U.*dety - V.*dmuy) .* bsxfun(@hypot,mu,eta);  %Vx
pV = -(U.*detx - V.*dmux) .* bsxfun(@hypot,mu,eta); %Vy

pU(1,:) = 0;
pV(1,:) = 0;

%   Creates V-vctr from Vx and Vy
VEL=zeros(size(pU,1),NX,'gpuArray');
...
VEL(1:size(pU,1),1:round(NX/2)) = bsxfun(@hypot,pU(1:size(pU,1),1:round(NX/2)), pV(1:size(pU,1),1:round(NX/2)));
VEL(1:size(pU,1),round(NX/2):NX) = bsxfun(@hypot,pU(1:size(pU,1),round(NX/2):NX), pV(1:size(pU,1),round(NX/2):NX)).*sign(pU(1:size(pU,1),round(NX/2):NX));
