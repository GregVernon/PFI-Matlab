function PFI_CUDA(SimType,Xmin,Xmax,Ymin,Ymax,Nx,My,Re,A,Ot,report,dt,Tol,IBL,...
...
%             end
        ...
        Omega0 = Omega;
        %%  Inline Vectorized Omega Calc %%
        ii = 2:Nx+1;
        jj = 2:My+1;
        Omega2 = arrayfun(@OMEGACALC,Omega0(jj,ii),Omega0(jj,ii+1),...
            Omega0(jj,ii-1),Omega0(jj+1,ii),Omega0(jj-1,ii),u(jj,ii+1),...
            u(jj,ii-1),v(jj+1,ii),v(jj-1,ii),Cx2,Cy2,d,dip1,dim1,djp1,...
                                                 djm1,alpha,alphaX,alphaY);
        Omega(jj,ii) = Omega2;
        %% Vectorized PsiCalc %%
        PsiTol = 1;
        kPsi = 0;
        while PsiTol > Tol
            kPsi = kPsi+1;
            Psi0 = Psi;
            [Psi2] = arrayfun(@PSICALC,Omega(jj,ii),Psi0(jj,ii+1),...
                Psi0(jj,ii-1),Psi0(jj+1,ii),Psi0(jj-1,ii),d2(jj-1,ii-1),...
                dxx,KappaA,Kappa2);
            Psi(jj,ii) = Psi2;
            % MATLAB 2011b code
...
%             for ii = 2:Nx-1
%                 for jj = 2:My-1
%                     Psi(jj,ii) = (dy^2*Psi(jj-1,ii)+dy^2*Psi(jj+1,ii)+dx^2*Psi(jj,ii-1)*dx^2*Psi(jj,ii+1)+dx^+dy^2*omega(jj,ii))/(2*(dx^2+dy^2));
%                 end
%             end
%             
%         end
...
%             end
        ...
        jj = 2:Nx+1;
        ii = 2:My+1;
        [u2,v2] = arrayfun(@VELCALC,Psi(ii+1,jj),Psi(ii-1,jj),Psi(ii,jj+1),...
            Psi(ii,jj-1),d3(ii,jj),dx2,dy2);
        u(ii,jj) = u2;
        v(ii,jj) = v2;
        %% Check Max Value Change %%
        % Matlab 2011b version
        ...
        if tick==psave
            if k~=Ot
                ...
                ct=ct+1;
%                 disp(datestr(clock))
                [STR2]=WriteFile(Nx,My,gather(Psi),gather(Omega),...
                    gather(u),gather(v),k,kerr,IBL,Re,gather(OmTol),...
                    gather(PsiTol),dx,dy,dt,ct,Tol,Xmin,Xmax,Ymin,Ymax,...
        ...
        if k==kerr
            STR1=[STR1;{[num2str(k,'%i'),'   ', num2str(kPsi),'    ',num2str(gather(OmTol),...
                '%3e'),'   ' num2str(gather(PsiTol),'%3e'),'   ',...
...
%             disp([k,OmTol,PsiTol,kerr])
        ...
        if k==Ot
            ...
            drawnow expose update
%             disp(datestr(clock))
            [STR2]=WriteFile(Nx,My,gather(Psi),gather(Omega),gather(u),...
                gather(v),k,kerr,IBL,Re,gather(OmTol),gather(PsiTol),dx,...
                dy,dt,ct,Tol,Xmin,Xmax,Ymin,Ymax,A,filename,NumOut,STR2);
            if AVG_DATA
                WriteAvgFile(Nx,My,gather(avgdata),k,kerr,IBL,Re,...
                    gather(OmTol),gather(PsiTol),dx,dy,dt,ct,Tol,Xmin,...
...
Omegawrite=gather(Omega);
Psiwrite=gather(Psi);
uwrite=gather(u);
vwrite=gather(v);

RESULT=[Omegawrite;Psiwrite;uwrite;vwrite;OUTIN;OUTDP]; %#ok<NASGU>
